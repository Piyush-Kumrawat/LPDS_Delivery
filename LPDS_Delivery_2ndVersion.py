...
def run_script(input_file, output_file):
    ...
    df_final = df_final.sort_values(by='csid', key=lambda x: x)

    # Drop the temporary 'csid' column used for sorting
    df_final.sort_values(by='Priority', inplace=True)  # Sort by 'Priority' column as well if needed
    df_final = df_final.drop(columns=['csid'])
